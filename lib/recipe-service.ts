...
export async function saveRecipe(url: string, userId: string): Promise<Recipe> {
  try {
    ...
    analytics.trackRecipeAdded(parsedRecipe.title);
...
export function searchRecipes(recipes: Recipe[], query: string): Recipe[] {
  ...
  return recipes.filter((recipe) => {
    ...
    if (recipe.ingredients.some((i) => i.toLowerCase().includes(searchTerm)))
      return true;

    // Search in categories
    if (recipe.categories.some((c) => c.toLowerCase().includes(searchTerm)))
      return true;

    // Search in comments
    if (recipe.comments && recipe.comments.toLowerCase().includes(searchTerm))
      return true;
